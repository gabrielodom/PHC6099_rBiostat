...
MDIAB82 = Age_Diagnosed = Age at which diabetes is diagnosed.
...
Mann_W_U$MDIAB82 <- as.numeric(Mann_W_U$MDIAB82)
class(Mann_W_U$MDIAB82)

Mann_W_U$SEX8 <- as_factor(Mann_W_U$SEX8)
class(Mann_W_U$SEX8)
...
Des <- 
 Mann_W_U %>% 
 select(Gender, Age_diagnosed) %>% 
 group_by(Gender) %>%
 summarise(
   n = n(),
   mean = mean(Age_diagnosed, na.rm = TRUE),
   sd = sd(Age_diagnosed, na.rm = TRUE),
   stderr = sd/sqrt(n),
   LCL = mean - qt(1 - (0.05 / 2), n - 1) * stderr,
   UCL = mean + qt(1 - (0.05 / 2), n - 1) * stderr,
   median = median(Age_diagnosed, na.rm = TRUE),
   min = min(Age_diagnosed, na.rm = TRUE), 
   max = max(Age_diagnosed, na.rm = TRUE),
   IQR = IQR(Age_diagnosed, na.rm = TRUE),
   LCLmed = MedianCI(Age_diagnosed, na.rm = TRUE)[2],
   UCLmed = MedianCI(Age_diagnosed, na.rm = TRUE)[3]
 )
...
```{r}
 ...
 summarise(
   `W Stat` = shapiro.test(Age_diagnosed)$statistic,
    p.value = shapiro.test(Age_diagnosed)$p.value,
   options(scipen = 999)
 )
```
